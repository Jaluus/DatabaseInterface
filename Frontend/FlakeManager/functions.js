...
function createTabelRow(data_dict) {
    ...
    // create each cell
    var index_cell = $("<td>").text(data_dict.flake_id).css("font-weight", "bold")
    ...
    var view_button = $('<a>').attr("data-bs-toggle", "modal").attr("data-bs-target", `#${modal_id}`).addClass("btn btn-primary btn-xs").text("View").attr('id', `view_button${data_dict.flake_id}`);
    //var toggle_button = $('<a>').addClass("btn btn-warning btn-xs").text("Toggle used")
    var delete_button = $('<a>').addClass("btn btn-danger btn-xs").text("Delete").attr('id', `delete_button${data_dict.flake_id}`).click({ 'id': data_dict.flake_id }, deleteHandler)
...
function createModal(data_dict) {
    ...
    var delete_button = $("<button>").attr({ "type": "button", 'data-bs-dismiss': "modal" }).addClass("btn btn-danger").text("Delete").click({ 'id': data_dict.flake_id }, deleteHandler)
    ...
    //header design
    var heading = $('<h4>').addClass("modal-title").text(`Viewing Flake ${data_dict.flake_id}`)
    ...
    <div class="row">
        <div class="col">    
            <img src="${image_directory}/eval_img.png" class="img-thumbnail" loading="lazy">
            <img src="${image_directory}/5x.png" class="img-thumbnail" loading="lazy">
            <img src="${image_directory}/20x.png" class="img-thumbnail" loading="lazy">
        </div>
        <div class="col">
            <img src="${image_directory}/2.5x.png" class="img-thumbnail" loading="lazy">
            <img src="${image_directory}/50x.png" class="img-thumbnail" loading="lazy">
            <img src="${image_directory}/100x.png" class="img-thumbnail" loading="lazy">
        </div>
    </div>`
